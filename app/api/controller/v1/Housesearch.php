...
class Housesearch extends Controller{
	...
	public function search(){
		...
		if ((($floor_area_min==='0')||(!empty($floor_area_min)))&&(($floor_area_max==='0')||(!empty($floor_area_max)))) {
				...
				$res = $house->field(['id','title','house_price','house_address','decoration_type','init_status','brokerage_plan','cover_img'])->where($str)->where('floor_area','between',"$floor_area_min,$floor_area_max")->where('title','like',"%$search%")->select();
				$data = [];
				foreach ($res as $k => $v) {
					//判断房源类型，如果是新房，则查出佣金
					if ($init_status == 1) {
		...
		//先执行进行标题查询
		$house = model('HouseSource');
		$res = $house->field(['id','title','house_price','house_address','decoration_type','init_status','brokerage_plan','cover_img'])->where($str)->where('title','like',"%$search%")->select();
		$data = [];
		foreach ($res as $k => $v) {
			//判断房源类型，如果是新房，则查出佣金
			if ($init_status == 1) {
